 ...
 * @LastEditors: minerva Minerva1_2@outlook.com
 * @LastEditTime: 2025-10-22 19:23:45
...
int ATSendCmd(char *buf, char *respon, uint32_t timeout)
{
    ...
    if(ret)
    {
        ...
        if(!at_state && respon)
        {
			copy_len = strlen(recv_buf) > AT_RESP_LEN ? AT_RESP_LEN : strlen(respon);
			/** 将获得的数据拷贝到用户提供的缓冲区 */
